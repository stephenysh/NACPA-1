...
from examples.oracle_4rwn_mydata import MyDataset
...
def closure():
    ...
    global final_ssim, final_ssim_max,  test_img_root, final_ssim_max_record
    ...
    if train_average_psnr == 0:
        ...
        train_average_psnr = psnr_noisy
    print('Iteration %05d lr: %f, Loss %f , PSNR_noisy: %f, PSNR_noisy_max: %f ' %
          (i, LR, average_loss, psnr_noisy, train_average_psnr))
    ...
    if i % 100 == 0:
        ...
        with torch.no_grad():
            ...
            print('Train set test %05d lr: %f, test psnr: %f , test max psnr: %f' %
                  (i, LR, psnr_1, temp_test_average_psnr))
            average_psnr = 0
            average_ssim = 0
            for idx_, img_name in enumerate(test_set.img_paths):
                ...
                test_noisy_img_torch = np_to_torch(test_img_np).type(dtype)
                test_out = torch_to_np(net(test_noisy_img_torch))

                final_reuslt = test_out.transpose(1, 2, 0)
...
img_gt = '/raid/huangyuan/sidd/sidd_small/gt_5000/'

test_img_root = './data/4rwn_test/'
test_gt = './data/4rwn_gt/'

psnr_record_iter = []
psnr_max_record = []
final_ssim_max_record = []
output_dir = 'results/rcan_4rwn/'
...
train_set = MyDataset(img_root, img_gt, fixed_shape, multilevel_noise=False, TEST=False)
net_input = torch.utils.data.DataLoader(train_set, batch_size=20, shuffle=True, num_workers=0)
test_set = MyDataset(test_img_root, test_gt, fixed_shape, multilevel_noise=False, TEST=True)
