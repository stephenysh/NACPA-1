...
TEST_ON_GRAY = False
test_img_root = '/raid/huangyuan/sidd/sidd_benchmark_validation_noisy_png/'
test_gt = '/raid/huangyuan/sidd/sidd_benchmark_validation_gt_png/'
dataset_name = 'sidd_val'
...
with torch.no_grad():
    ...
    for idx_, img_name in enumerate(test_set.img_paths):
        ...
        test_gt_img = Image.open(os.path.join(test_gt, img_name)).convert('RGB')
        test_gt_np = pil_to_np(test_gt_img)
        ...
        psnr_2 = compare_psnr(test_gt_np.transpose(1,2,0), np.clip(final_reuslt, 0, 1))
        final_ssim = compare_ssim(test_gt_np.transpose(1, 2, 0), np.clip(final_reuslt, 0, 1), data_range=1,
                                  multichannel=True)

        tmp_name_p = f'{img_name[:-4]}_{psnr_2:.2f}_final_{final_ssim:.4f}'
        # tmp_name_p = img_name[:-4]
        save_image(tmp_name_p, np.clip(final_reuslt.transpose(2, 0, 1), 0, 1), output_path=output_dir)
        print('%s , psnr: %f, ssim: %f'%(img_name, psnr_2, final_ssim))
        average_psnr += psnr_2
